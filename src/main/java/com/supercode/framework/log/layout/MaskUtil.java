...
public class MaskUtil {
    ...
    private final static String REPLACEMENT = "Sensitive ({0})******";

    private static final Map<String, String> REGEX_MAP = new ConcurrentHashMap<>();

    private static final Map<String, Pattern> REGEX_PATTEN_MAP = new ConcurrentHashMap<>();

    private static final Map<String, Boolean> HASH_OUTPUT = new ConcurrentHashMap<>();
    ...
    private static void initPattern(int num, String line, String resourceName) {
        ...
        REGEX_MAP.put(resourceName + "#" + num, line);
        REGEX_PATTEN_MAP.put(line, Pattern.compile(line, Pattern.DOTALL));
    }
        ...
        for (String line : plainTextPatterns) {
            ...
            if (length > 1 && line.endsWith("#")) {
                if (line.charAt(length - 2) == '\\') {
                    ...
                    line = line.substring(0, length - 1);
                    HASH_OUTPUT.put(line, Boolean.TRUE);
                }
            }
    ...
    private static void doMask(StringBuilder logBuf, int num, String regex) {
        try {
            Pattern p = REGEX_PATTEN_MAP.get(regex);
            if (p == null) {
                p = Pattern.compile(regex, Pattern.DOTALL);
            }
            Matcher m = p.matcher(logBuf);
            if (m.matches()) {
                if (Boolean.TRUE.equals(HASH_OUTPUT.get(regex))) {
            ...
            LOGGER.error("error in masking sensitive info", e);
        }
    }
